    ...
    this.bot.on('callback_query', async (query) => {
      ...
      if (action.match(/(remove_wallet|detail_wallet|change_input_token) (\S+)/g)) {
        ...
        switch (type) {
          ...
          case 'remove_wallet': {
            const sent = await this.bot.sendMessage(chatId, "⚠️  Are you sure?  type 'yes' to confirm ⚠️", {
      ...
      if (action.match(/(whale_wallet_remove|whale_wallet_detail|holding_token|analysis_wallet) (\S+)/g)) {
        const [type, address] = action.split(' ');
        switch (type) {
      ...
      switch (action) {
        ...
        // MARK: Import wallet
        case IMPORT_WALLET: {
          const replyMsg = await this.bot.sendMessage(chatId, 'Imput your secret key or mnemonic here:', {
            reply_markup: { force_reply: true },
          });
