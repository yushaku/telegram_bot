...
export async function transformRoutesToTrade(
  ...
  const showUniswapXTrade =
    ...
    (routerPreference === RouterPreference.X ||
      (isUniswapXDefaultEnabled && routerPreference === RouterPreference.API));
  ...
  const usdCostPerGas =
    gasUseEstimateUSD && gasUseEstimate
      ? gasUseEstimateUSD / gasUseEstimate
      : undefined;
  ...
  if (
    isUniswapXBetter &&
    (routerPreference === RouterPreference.X || isUniswapXDefaultEnabled)
  ) {
